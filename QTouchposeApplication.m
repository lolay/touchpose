...
#define DEFAULT_BACKGROUND_ALPHA 0.8
#define DEFAULT_BORDER_ALPHA 0.6
...
- (id)initWithPoint:(CGPoint)point borderColor:(UIColor*) borderColor backgroundColor:(UIColor*) backgroundColor;
...
{
	return [self initWithPoint:CGPointMake(0.0, 0.0) borderColor:[UIColor colorWithWhite:1.0 alpha:DEFAULT_BORDER_ALPHA] backgroundColor:[UIColor colorWithWhite:1.0 alpha:DEFAULT_BACKGROUND_ALPHA]];
}
...
- (id)init
{
    if ((self = [super init]))
    {
		...
		_borderColor = [UIColor colorWithWhite:1.0 alpha:DEFAULT_BORDER_ALPHA];
		_backgroundColor = [UIColor colorWithWhite:1.0 alpha:DEFAULT_BACKGROUND_ALPHA];
		_animates = YES;
...
- (void)removeTouchesActiveTouches:(NSSet *)activeTouches
{
    ...
    for (CFIndex i = 0; i < count; ++i)
    {
        ...
        if (activeTouches == nil || ![activeTouches containsObject:touch])
        {
			...
			if (self.animates) {
				[UIView animateWithDuration:self.animationDuration animations:^{ view.alpha = 0.0f; } completion:^(BOOL completed){ [view removeFromSuperview]; }];
			} else {
				view.alpha = 0.0;
    ...
    for (UITouch *touch in touches)
    {
        ...
        if (touch.phase == UITouchPhaseCancelled || touch.phase == UITouchPhaseEnded)
        {
            ...
            if (fingerView != NULL)
            {
                ...
                CFDictionaryRemoveValue(_touchDictionary, (__bridge const void *)(touch));
				if (self.animates) {
					[UIView animateWithDuration:self.animationDuration animations:^{ fingerView.alpha = 0.0f; } completion:^(BOOL completed){ [fingerView removeFromSuperview]; }];
				} else {
					fingerView.alpha = 0.0;
